...
try:
    from twisted.conch.ssh import (userauth, transport, channel, connection,
                                   session, keys)
    from twisted.conch.manhole_ssh import TerminalUser, TerminalSession, TerminalRealm, TerminalSessionTransport, \
        ConchFactory
except ImportError:
    ssh = False
else:
    ...
    class TestConnection(connection.SSHConnection):
        ...
        def serviceStarted(self):
            self.__channel = SessionChannel(self.protocolFactory, self.protocolArgs, self.protocolKwArgs, self.width,
                                            self.height)
    ...
    class TestTransport(transport.SSHClientTransport):
        ...
        def connectionSecure(self):
            self.__connection = TestConnection(self.protocolFactory, self.protocolArgs, self.protocolKwArgs, self.width,
                                               self.height)
            self.requestService(
