...
class TkConchMenu(Tkinter.Frame):
    def __init__(self, *args, **params):
        ...
        Tkinter.Button(self, text='Connect!', command=self.doConnect).grid(column=1, columnspan=3, row=14,
                                                                           sticky='nesw')
...
%s key fingerprint is %s.""" % (host,
    ...
    def _cbVerifyHostKey(self, ans, pubKey, khHost, keyType):
        if ans.lower() not in ('yes', 'no'):
            return deferredAskFrame("Please type  'yes' or 'no': ", 1).addCallback(self._cbVerifyHostKey, pubKey,
                                                                                   khHost, keyType)
        if ans.lower() == 'no':
